...
void ClientDamage( gentity_t *clent, int entnum, int enemynum, int id ) {
	...
	case CLDMG_DEBRIS:
		...
		break;
	case CLDMG_SPIRIT:
		if ( enemy->aiCharacter == AICHAR_ZOMBIE || enemy->aiCharacter == AICHAR_ZOMBIE_SURV || enemy->aiCharacter == AICHAR_ZOMBIE_FLAME || enemy->aiCharacter == AICHAR_ZOMBIE_GHOST  ) {
	...
	case CLDMG_FLAMETHROWER:
		...
		if ( ( enemy->aiCharacter == AICHAR_ZOMBIE || enemy->aiCharacter == AICHAR_ZOMBIE_FLAME || !Q_stricmp( enemy->classname, "props_flamethrower" ) ) && ent->takedamage && !AICast_NoFlameDamage( ent->s.number ) ) {
