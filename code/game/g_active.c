...
void ClientEvents( gentity_t *ent, int oldEventSequence ) {
	...
	gclient_t   *client;
	int damage;
	int stunTime = 0;		//----(SA)	added
	float fallSoundMul = 1.0f;
...
*/
	...
	if ( !G_IsEntityAI( ent ) ) { // don't drop bots
...
//	client->latched_wbuttons |= client->wbuttons & ~client->oldwbuttons;	// FIXME:? (SA) MP method
	...
	// funstats, calculate the time in the air
...
// new way, based on constant spawnrates
		...
		*/
		...
		if ( ( g_maxlives.integer > 0 ) && ent->client->ps.persistant[PERS_RESPAWNS_LEFT] == 0 ) {
			// if no lives left, should go to limbo ?
			limbo( ent, qtrue );
			return;
		}
