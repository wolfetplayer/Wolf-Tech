...
//	}
	...
	} else if ( ent->aiCharacter == AICHAR_ZOMBIE_GHOST ) {
		SET_FLAMING_ZOMBIE( ent->s, 0 );
	}
	//
	// update bounding box
	//AIChar_SetBBox( ent, cs, qtrue );
	// a dedicated server can't use the trap_GetTag functions, this function does
...
qboolean AICast_GetAvoid( cast_state_t *cs, bot_goal_t *goal, vec3_t outpos, qboolean reverse, int blockEnt ) {
	...
	float angleDiff;
	int starttraveltime = 0, traveltime = 0;
	int invert;
	float inc;
...
void AICast_QueryThink( cast_state_t *cs ) {
	...
	// FIXME Had a segfault at the line "if ( ocs->lastWeaponFired > cs->queryStartTime ) {"
	if ( !ent || !ocs || !cs ) {
		return;
	}
