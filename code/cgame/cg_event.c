...
static void CG_Obituary( entityState_t *ent ) {
	...
	// Out of range, what killed the target?
...
void CG_PopUpClipboard( centity_t *activator, int density ) {
	...
	// Only show the clipboard on the client who activated it.
...
==============
CG_PlayerDied
Since now we have a proper event for when a player dies, we could move
...
void CG_PlayerDied( centity_t *entity, centity_t *killer ) {
	...
	// Only close the clipboard on the client who died.
...
// for debug text. these need to match the order of debugText_t
char *classNameMap[] = {
	"coop_spawnpoint",
	"ai_trigger"
};
	...
	case EV_FOOTSTEP:
		...
		if ( cg_footsteps.integer ) {
			if ( cent->currentState.aiChar == AICHAR_ELITEGUARD ) {
				...
				trap_S_StartSound( NULL, es->number, CHAN_BODY, cgs.media.footsteps[ FOOTSTEP_ZOMBIE_STEP ][footstepcnt] );
			} else if ( cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) {
				trap_S_StartSound( NULL, es->number, CHAN_BODY, cgs.media.footsteps[ FOOTSTEP_ZOMBIE_STEP ][footstepcnt] );
