...
void CG_BloodTrail( localEntity_t *le ) {
	...
	if ( cent && cent->currentState.aiChar == AICHAR_ZOMBIE_SURV ) {
		step = 30;
	}
	...
	if ( cent && cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) {
		step = 30;
	}
	...
	for ( ; t <= t2; t += step ) {
		...
		if (( cent && cent->currentState.aiChar == AICHAR_ZOMBIE ) || ( cent && cent->currentState.aiChar == AICHAR_ZOMBIE_SURV ) || ( cent && cent->currentState.aiChar == AICHAR_ZOMBIE_FLAME ) || ( cent && cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) ) {
